...
def addIfRequired (result, sg, quality, task, check_for_duplicates=False):
    if (quality > task.minQuality):
        if (check_for_duplicates and (quality, sg) in result):
            return
        if (len(result) < task.resultSetSize[0]):
...
# for bi-cases
def addIfRequired_Bicases (result, sg1, sg2, quality, task, check_for_duplicates=False):
    if (quality > task.minQuality):

        last_result = result.copy()
        overall_sg1_dup = [i[1] for i in last_result]
        overall_sg1_dup.append(sg1)
        overall_sg1 =[i for n,i in enumerate(overall_sg1_dup) if i not in overall_sg1_dup[:n]]
        # print(len(overall_sg1))

        if (check_for_duplicates and (quality, sg1, sg2) in result):
            return
        if (len(overall_sg1) <= task.resultSetSize[1]):
            heappush(result, (quality, sg1, sg2))
        elif (quality > result[0][0]):
            heappop(result)
            heappush(result, (quality, sg1, sg2))
...
def equalFrequencyDiscretization (data, attributeName, nbins=5, weightingAttribute=None):
    ...
    #     Final_cutpoints.append((cutpoints[i]+cutpoints[i+1])/2)
